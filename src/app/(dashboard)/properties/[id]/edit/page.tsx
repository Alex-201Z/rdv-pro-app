...
import { Property, Seller, PropertyType } from '@/types/real-estate';
...
export default function EditPropertyPage() {
  ...
  const loadProperty = async () => {
    try {
      ...
      setFormData({
        seller_id: propertyData.seller_id?.toString() || '',
        ...
        agency_fees: propertyData.agency_fees?.toString() || '',
        // Commission fields
        commission_percentage: propertyData.commission_percentage?.toString() || '',
        entered_by_professional: propertyData.entered_by_professional || false,
        sold_by_professional: propertyData.sold_by_professional || false,
        // Pipeline dates
        registration_date: propertyData.registration_date ? new Date(propertyData.registration_date).toISOString().slice(0, 16) : '',
        first_estimate_date: propertyData.first_estimate_date || '',
        estimate_delivery_date: propertyData.estimate_delivery_date || '',
        mandate_signature_date: propertyData.mandate_signature_date || '',
        offer_received_date: propertyData.offer_received_date || '',
        buyer_offer_name: propertyData.buyer_offer_name || '',
        sales_agreement_date: propertyData.sales_agreement_date || '',
        authentic_deed_date: propertyData.authentic_deed_date || '',
        keys_handover_date: propertyData.keys_handover_date || '',
      });
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const submitData = {
      ...
      // Commission fields
      commission_percentage: formData.commission_percentage ? Number(formData.commission_percentage) : undefined,
      entered_by_professional: formData.entered_by_professional,
      sold_by_professional: formData.sold_by_professional,
      // Pipeline dates
      registration_date: formData.registration_date || undefined,
      first_estimate_date: formData.first_estimate_date || undefined,
      estimate_delivery_date: formData.estimate_delivery_date || undefined,
      mandate_signature_date: formData.mandate_signature_date || undefined,
      offer_received_date: formData.offer_received_date || undefined,
      buyer_offer_name: formData.buyer_offer_name || undefined,
      sales_agreement_date: formData.sales_agreement_date || undefined,
      authentic_deed_date: formData.authentic_deed_date || undefined,
      keys_handover_date: formData.keys_handover_date || undefined,
    };

    try {
      setLoading(true);
      await propertiesApi.update(Number(params.id), submitData);
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          <h2 className="text-lg font-semibold text-white mb-4">💰 Commission et Honoraires</h2>
          <div className="space-y-4">
            <Input
            ...
            {calculatedFees > 0 && (
              <div className="p-4 bg-dark-800/50 rounded-lg border border-dark-700">
                <h3 className="text-sm font-semibold text-dark-300 mb-3">Calculs automatiques</h3>
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-dark-700">
                    ...
                    <span className="text-lg font-semibold text-green-400">
                      {netSeller.toLocaleString('fr-FR')} €
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="pt-4 space-y-2">
              <p className="text-sm text-dark-400 mb-2">Commission professionnelle:</p>
              <label className="flex items-center gap-2 text-dark-300 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="w-4 h-4 rounded border-dark-700 bg-dark-800 text-primary-500 focus:ring-primary-500"
                />
                <span>Bien rentré par le professionnel (+25%)</span>
              </label>
              <label className="flex items-center gap-2 text-dark-300 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="w-4 h-4 rounded border-dark-700 bg-dark-800 text-primary-500 focus:ring-primary-500"
                />
                <span>Bien vendu par le professionnel (+25%)</span>
              </label>
            </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <span className="text-xl font-bold text-yellow-400">
                      {professionalCommission.toLocaleString('fr-FR')} €
                    </span>
